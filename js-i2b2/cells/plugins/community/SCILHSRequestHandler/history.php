...
if(result_status_success($response_xml)){
	...
	$output .= "<table id='SCILHSRequestHandler-HistoryResults' cellspacing='0'>\n";
	if(isAdmin($user_id)){
	...
	foreach ($files as $mtime => $jobfile){
		...
		switch($data->status){
			...
			case 'PROCESSING':
				if($time_diff > 10800){
					$status = '<img src="js-i2b2/cells/plugins/community/SCILHSRequestHandler/assets/warning.gif" align="absbottom"/> <em>Timed Out</em><br/>Processed: ' . $data->payload . " [<a href=\"#\" onclick=\"i2b2.SCILHSRequestHandler.cancelJob('".$job_id."');return false;\">Cancel</a>]";
				} else {
					$status = '<img src="js-i2b2/cells/plugins/community/SCILHSRequestHandler/assets/ajaxicon.gif" align="absbottom"/> Processing: ' . $data->payload . " [<a href=\"#\" onclick=\"i2b2.SCILHSRequestHandler.cancelJob('".$job_id."');return false;\">Cancel</a>]";
				}
				break;
			case 'FINISHED':
				$status = "<strong>Completed: </strong><a style=\"color:#000;text-decoration:underline;font-weight:normal;\" href=\"#\" onclick=\"javascript:i2b2.SCILHSRequestHandler.downloadJob('".$job_id."');return false;\">Download</a>";
				if(isAdmin($user_id)){
					$status .= "<br/><a href=\"#\" style=\"color:#000;text-decoration:underline;font-weight:normal;\" onclick=\"i2b2.SCILHSRequestHandler.rerunJob('".$job_id."');return false;\">Re-run</a>";
				}
				break;
			case 'CANCELLED':
