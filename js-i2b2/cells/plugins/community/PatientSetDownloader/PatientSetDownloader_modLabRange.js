 ...
 * @inherits 	i2b2.PatientSetDownloader.view
 * @namespace	i2b2.PatientSetDownloader.view.modalLabValues
 * @author		Taowei David Wang, Nich Wattanasin
 * @version 	1.0 7/12/16
 * ----------------------------------------------------------------------------------------
 * updated 2/16/2015: Taowei David Wang 
 * modified to suit its own set of HTML elements (see injected_screens.html) instead of sharing with the query tool. the sharing approach broke things.
 */

i2b2.PatientSetDownloader.view.modalLabValues = {
	formdata: {},
	cfgTestInfo: {
		name: 'RND-TEST',
		flagType: 'NA',
	...
	updateValue: function(e) {
	    ...
	    try {
	        ...
	        if ((sTargAnchor == 'PSV_lblToxL') || (sTargAnchor == 'PSV_lblLofL') || (sTargAnchor == 'PSV_lblHofL')) {
	            ...
	            i2b2.PatientSetDownloader.view.modalLabValues.formdata.numericOperator = 'LE';
	        ...
	        if ((sTargAnchor == 'PSV_lblToxH') || (sTargAnchor == 'PSV_lblLofH') || (sTargAnchor == 'PSV_lblHofH')) {
	            ...
	            i2b2.PatientSetDownloader.view.modalLabValues.formdata.numericOperator = 'GE';
	            $('PSV_mlvfrmNumericValue').value = sTargNumber;
	        }
	    }
	    catch (eError) {
...
// ================================================================================================== //
	show: function(queryPanelController, key, conceptObj, isModifier) {
	    var fd = i2b2.PatientSetDownloader.view.modalLabValues.formdata;
		this.cpc = queryPanelController;
		i2b2.PatientSetDownloader.view.modalLabValues.isModifier = isModifier;
		this.key = key;
		this.conceptObj = conceptObj; // = i2b2.PatientSetDownloader.view.modalLabValues.conceptObj
		...
		if (!this.sd) {
		    this.sd = new YAHOO.widget.SimpleDialog("PSV_itemLabRange", {
				...
				buttons: [{
					...
					handler: 
						(function() {
						    var closure_qpi = i2b2.PatientSetDownloader.view.modalLabValues.qpi;
							var closure_cpc = i2b2.PatientSetDownloader.view.modalLabValues.cpc;
							var closure_key = i2b2.PatientSetDownloader.view.modalLabValues.key;
							var closure_number = i2b2.PatientSetDownloader.view.modalLabValues.itemNumber;
							// submit value(s)
							if (this.submit()) {
							    if (i2b2.PatientSetDownloader.view.modalLabValues.isModifier) 
                                {
							        i2b2.PatientSetDownloader.view.modalLabValues.conceptObj.valueRestriction = i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.ModValues;
                                } 
                                else 
                                {
							        i2b2.PatientSetDownloader.view.modalLabValues.conceptObj.valueRestriction = i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.LabValues;
								}
							    ...
							    delete i2b2.PatientSetDownloader.view.modalLabValues.isModifier;
							}
						})
					}, {
					text: "Cancel",
			...
			YAHOO.util.Event.addListener("PSV_mlvfrmStrValue", "keypress", (function(e) {
				...
				var msl = i2b2.PatientSetDownloader.view.modalLabValues.cfgTestInfo.valueValidate.maxString;
				if (!msl || this.value.length < msl) {
				    delete i2b2.PatientSetDownloader.view.modalLabValues.formdata.lastValidStr;
					return true;
				} else {
				    if (!i2b2.PatientSetDownloader.view.modalLabValues.formdata.lastValidStr) {
				        i2b2.PatientSetDownloader.view.modalLabValues.formdata.lastValidStr = this.value;
					}
					return true;
				}
			...
			YAHOO.util.Event.addListener("PSV_mlvfrmStrValue", "keyup", (function(e) {
				// anonymous function
			    if (i2b2.PatientSetDownloader.view.modalLabValues.formdata.lastValidStr) {
			        this.value = i2b2.PatientSetDownloader.view.modalLabValues.formdata.lastValidStr;
				}
			}));
			this.sd.hideEvent.subscribe(function(o) {
				// setTimeout(function() {panel.destroy();}, 0);
				var dm = i2b2.PatientSetDownloader.view.modalLabValues.cfgTestInfo;
				if (dm.valueType) {
					if(dm.valueType=="GENOTYPE"){
						...
						if(!existingInput){
							...
							i2b2.PatientSetDownloader.sanitizeGenomicConcepts();
							//queryPanelController._deleteConcept(i2b2.PatientSetDownloader.view.modalLabValues.htmlID,"GTPAutoDelete");
						}
						else
						{
							if(!i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.LabValues)
								queryPanelController._deleteConcept(i2b2.PatientSetDownloader.view.modalLabValues.htmlID,"GTPAutoDelete");
						}
					}
				}
...
// ================================================================================================== //
	changeHandler: function(e) {
		...
		tn.blur();
		// save the changes
		i2b2.PatientSetDownloader.view.modalLabValues.formdata = fd;
		i2b2.PatientSetDownloader.view.modalLabValues.Redraw();
	},
	
// ================================================================================================== //
	cfgByMetadata: function(refXML,conceptCode){
		// load and process the xml info
	    i2b2.PatientSetDownloader.view.modalLabValues.formdata.ignoreChanges = true;
	    var dm = i2b2.PatientSetDownloader.view.modalLabValues.cfgTestInfo;
	    var fd = i2b2.PatientSetDownloader.view.modalLabValues.formdata;
		fd.selectedType= "NONE";
        ...
        else if (i2b2.PatientSetDownloader.view.modalLabValues.isModifier) 
        {
		...
		jQuery('#PSV_frmEnterRSIDValue').on('keypress, keydown', function(event) {
			if ((event.which != 37 && (event.which != 39))
				...
				if(i2b2.PatientSetDownloader.view.modalLabValues.isTextSelected(document.getElementById('PSV_frmEnterRSIDValue')))
				{
					$('PSV_frmEnterRSIDValue').value = 'rs';
					return false;
				}
		...
		i2b2.PatientSetDownloader.view.modalLabValues.formdata.ignoreChanges = false;
		i2b2.PatientSetDownloader.view.modalLabValues.setUnits();
		i2b2.PatientSetDownloader.view.modalLabValues.Redraw();
	},
...
// ================================================================================================== //
	Redraw: function(){
		if (i2b2.PatientSetDownloader.view.modalLabValues.formdata.ignoreChanges) return;
		i2b2.PatientSetDownloader.view.modalLabValues.formdata.ignoreChanges = true;
		var fd = i2b2.PatientSetDownloader.view.modalLabValues.formdata;
		var dm = i2b2.PatientSetDownloader.view.modalLabValues.cfgTestInfo;
...
// ================================================================================================== //
	ValidateSave: function() {
		...
		switch (fd.selectedType) {
			case "NONE":
			    if (i2b2.PatientSetDownloader.view.modalLabValues.isModifier) {
					delete i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.ModValues;
				} else {
					delete i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.LabValues;					
				}
				return true;
				break;
			case "FLAG":
				...
				switch(dm.valueType) {
					...
					case "GENOTYPE":
						...
						if(dm.searchByRsId){  //Validate rsid input
							...
							if(rsIdVal.indexOf('|')>0)
							{
								...
								for(var j = 1 ; j < rsIdSplit.length ; j++)
								{
									...
									{
										var consequence = i2b2.PatientSetDownloader.view.modalLabValues.GetConsequenceForRSId(thisInput);
										if(consequence)
										{
							...
							var zygsity = i2b2.PatientSetDownloader.view.modalLabValues.GetZygosities();
							if(zygsity.length==0)
						...
						if(dm.searchByGeneName)  //Validate gene name input
						{
							...
							var consequnce = i2b2.PatientSetDownloader.view.modalLabValues.GetConsequences();
		...
		if (i2b2.PatientSetDownloader.view.modalLabValues.isModifier) {
			if (tmpLabValue) {
				i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.ModValues = tmpLabValue;
			} else {
				delete i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.ModValues;
			}
		} else { 
			if (tmpLabValue) {
				i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.LabValues = tmpLabValue;
			} else {
				delete i2b2.PatientSetDownloader.view.modalLabValues.i2b2Data.LabValues;
			}
		}
